// Bamboo Control Node with gRPC - Simplified version of deoxys control node
// Supports only Joint Impedance control with Min-Jerk interpolation
// Uses gRPC instead of ZMQ for arm control communication
// Gripper communication still uses ZMQ (not modified)
...
private:
    ...
    // Control parameters
    const int policy_rate_ = 20;  // Hz
    const int traj_rate_ = 500;   // Hz
    const double traj_interpolator_time_fraction_ = 0.1;

    // Trajectory queue for sequential execution
    std::queue<FrankaControlMessage> trajectory_queue_;
    ...
    Status ExecuteJointImpedanceTrajectory(
        ...
        try {
            ...
            for (int i = 0; i < request->waypoints_size(); ++i) {
                const FrankaControlMessage& waypoint = request->waypoints(i);
                ...
                goal_q_ << ji_msg.goal().q1(), ji_msg.goal().q2(), ji_msg.goal().q3(),
                         ji_msg.goal().q4(), ji_msg.goal().q5(), ji_msg.goal().q6(),
                         ji_msg.goal().q7();

                std::cout << "[GRPC] Waypoint " << i+1 << "/" << request->waypoints_size()
                          << ": " << goal_q_.transpose() << std::endl;

                // Execute this waypoint
                bool success = executeWaypoint(waypoint);
                if (!success) {
                    response->set_success(false);
                    return Status(grpc::StatusCode::INTERNAL,
...
private:
    bool executeWaypoint(const FrankaControlMessage& waypoint) {
        ...
        interpolator_->Reset(
            ...
            policy_rate_,
            traj_rate_,
            traj_interpolator_time_fraction_
        );
        ...
        try {
            ...
            std::cerr << "[WAYPOINT] Control exception: " << e.what() << std::endl;
            control_running_ = false;
            return false;
        }
    }
