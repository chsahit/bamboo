...
JointImpedanceController::JointImpedanceController(franka::Model *model)
  ...
  // Gains from reference driver (franka_driver.cc
  // SetGainsForJointStiffnessErrorToTorque) Originally from libfranka's
  // joint_impedance_control demo, refined empirically
...
JointImpedanceController::Step(const franka::RobotState &robot_state,
  ...
  // Get dynamics from Franka model and copy to Eigen objects (matching
  // reference driver)
  ...
  // Eigen::VectorXd::Map(gravity_array.data(), 7);
  ...
  // tau = -Kp * (q_actual - q_cmd) - Kd * (dq_actual - dq_cmd) + M * a_cmd +
  // coriolis which is equivalent to: tau = Kp * error_q + Kd * error_dq + M *
  // a_cmd + coriolis NOTE: Franka already does automatic gravity compensation,
  ...
  if (debug_counter % 100 == 0) {  // Print every 100 cycles
    ...
    std::cout << std::endl;
    std::cout << "[DEBUG] M(0,0): " << M(0,0) << " M(1,1): " << M(1,1) << "
  M(6,6): " << M(6,6) << std::endl; std::cout << "[DEBUG] M diagonal: " <<
  M.diagonal().transpose() << std::endl; std::cout << "[DEBUG] Full M matrix:"
  << std::endl << M << std::endl; std::cout << "[DEBUG] desired_ddq: " <<
  desired_ddq.transpose() << std::endl; std::cout << "[DEBUG] M*desired_ddq: "
  << (M * desired_ddq).transpose() << std::endl; std::cout << "[DEBUG] coriolis:
