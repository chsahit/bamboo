...
- [x] Gripper state returns is_grasped and is_moving but client only uses width. Consider if these fields should be exposed to users or removed.
- [x] Controller sends dq, tau_J, time_sec in RobotState but client never uses them (only uses q and O_T_EE). Consider removing these fields or document why they're included.
